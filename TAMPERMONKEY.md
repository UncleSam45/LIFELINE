...
`lifeline-kindroid-call-toolkit.user.js` is the single, standalone call-page toolkit used by both Electron and Tampermonkey. Electron now loads that file through its dedicated Kindroid preload and evaluates it in the website's main JavaScript world, matching Tampermonkey's `@grant none` behavior. This runs in the side panel, GROUPMAKER call windows, and Kindroid-created child windows, including client-side call navigation that does not reload the page. On Kindroid call pages it mounts a large, collapsible quick-reply panel at the middle-left of the screen. Its starter replies can be sent with one click. The large text field sends on Enter (use Shift+Enter for a new line) and automatically encloses every on-the-fly message in asterisks. Open the gear menu to add, edit, or delete presets; changes are saved in browser storage and remain available after reloads. Preset messages continue to be sent exactly as saved.
