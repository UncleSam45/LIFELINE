...
Every Kindroid window opened by LIFELINE loads both root userscripts through an isolated preload at document idle, subject to their `@match` URL families. The scripts remain independent and are executed unchanged; the Electron host only provides the Tampermonkey APIs declared by the transcript bridge. Transcript extraction, scheduling, merging, and UI behavior therefore live exclusively in the userscript rather than in Electron main-process page manipulation.
