...
LIFELINE leaves ordinary Kindroid pages completely untouched. Dedicated call windows receive a small, sandboxed compatibility preload for only the Tampermonkey APIs declared by the transcript bridge; after the call document finishes loading, Electron executes the matching root userscripts unchanged in the page world. Node and Electron internals never enter the page. Transcript extraction, scheduling, merging, and UI behavior therefore live exclusively in the userscript rather than in Electron main-process page manipulation.
