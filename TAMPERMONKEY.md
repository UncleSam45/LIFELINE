...
The extractor recognizes both legacy Kindroid menus and the current `aria-label="Call menu"` dialog trigger. It prioritizes the current call-menu and `call-dock-v2_menu-row` controls, uses one native DOM click, verifies that the Transcript control or panel activated, and falls back to the control's accessible Enter-key handler only when activation did not occur. It retries on a later capture when rows remain unavailable and avoids toggling a control whose `aria-pressed` state says the panel is open. Token remembering is enabled by default and capture persists the token before doing any network work; **Forget token** or clearing **Remember in Tampermonkey** removes it. Browser warnings from Kindroid, LiveKit, Chargebee, or hCaptcha about mobile metadata and `aria-hidden` do not originate from this userscript.
