...
def test_family_memory_update_uses_generation_coparent_links_and_saves_bridge():
    ...
    assert "has a ${childRole(children[0])} named" in MAIN_JS
    assert "return 'daughter'" in MAIN_JS
    assert "return 'son'" in MAIN_JS
    assert "familySentences = [childOverview, ...coParentSentences].filter(Boolean)" in MAIN_JS
...
def test_all_directory_people_receive_stable_generation_profiles():
    ...
    assert "entries().forEach((entry) => ensureGenerationPerson(ensureEntry(entry)))" in MAIN_JS
...
def test_family_links_are_mirrored_on_directory_entries_before_navigation():
    ...
    assert "person.parents = [...savedRelations.parents]" in MAIN_JS
    assert "person.children = [...savedRelations.children]" in MAIN_JS
