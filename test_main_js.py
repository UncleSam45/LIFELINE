...
def test_journal_rows_are_reacquired_across_virtualized_list_scrolls():
    opener = JOURNAL_ADAPTER.split("async openJournalEntry", 1)[1].split("async readJournalEditor", 1)[0]
    assert "container.scrollTop = 0" in opener
    assert "for (let attempt = 0; attempt < 80" in opener
    assert "node.scrollIntoView" in opener
