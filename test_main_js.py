...
def test_family_memory_update_includes_pregnancy_status_and_progress():
    ...
    assert "${name} is currently pregnant${partnerPhrase}${progressPhrase}." in MAIN_JS
    assert "${name} is not currently pregnant." in MAIN_JS
...
const expected = "ARI ARTEBELLO is not currently pregnant.\\nARI ARTEBELLO has 2 daughters and 1 son. ARI ARTEBELLO's children are named ERENCINNI VAISICA A., JIANNA VAISICA A. and SANTINO VAISICA A.";
...
const partneredExpected = "ARI ARTEBELLO is not currently pregnant.\\nARI ARTEBELLO has 1 daughter with LEO. ARI ARTEBELLO's child with LEO is named VENUS.\\nARI ARTEBELLO has 1 daughter and 1 son with SAM VAISICA. ARI ARTEBELLO's children with SAM VAISICA are named MERCEDES and SANTINO.";
