...
def test_root_bridge_config_is_authoritative_and_every_save_is_verified():
    ...
    assert "async function verifyRootBridgeConfig(expectedDirectoryUids)" in MAIN_JS
    assert "Directory profile(s) are missing from root ${BRIDGE_PATH}" in MAIN_JS
    assert MAIN_JS.count("await verifyRootBridgeConfig(expectedDirectoryUids)") == 2
