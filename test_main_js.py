...
def test_family_memory_update_preserves_existing_memory_formatting():
    update_body = MAIN_JS.split("async function updateFamilyMemory(entry)", 1)[1].split("function bridgeUrl", 1)[0]
    assert "existingMemory = String(entry.ai_memory || '')" in update_body
    assert "previousGenerated = String(entry.family_memory_generated || '')" in update_body
    assert "existingMemory.replace(previousGenerated, generated.text)" in update_body
    assert "`${existingMemory}${existingMemory.endsWith('\\n') ? '' : '\\n'}${generated.text}`" in update_body
    assert "entry.family_memory_generated = generated.text" in update_body
    assert "memoryField.value = entry.ai_memory" in update_body
