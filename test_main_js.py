...
def test_family_relationship_changes_are_reciprocal_and_saved():
    ...
    assert "setGenerationRelations(generation, 'parents'" in MAIN_JS
    assert "setGenerationRelations(generation, 'children'" in MAIN_JS
    assert "refreshGenerationRelationshipDisplay(generation)" in MAIN_JS
...
setGenerationRelations(mercedes, 'parents', ['dir_ari', 'dir_sam']);
syncGenerationPeopleWithDirectory();
...
def test_root_bridge_config_is_authoritative_and_every_save_is_verified():
    ...
    assert "async function verifyRootBridgeConfig(expectedDirectoryProfiles)" in MAIN_JS
    assert "Directory profile(s) are missing from root ${BRIDGE_PATH}" in MAIN_JS
    assert "savedProfiles.get(uid) !== name" in MAIN_JS
    assert MAIN_JS.count("await verifyRootBridgeConfig(expectedDirectoryProfiles)") == 2
...
def test_frontend_state_is_local_first_and_github_writes_complete_snapshots():
    ...
    assert "persistLocalConfig('Startup restore from root config.json')" in MAIN_JS
    ...
    assert write_bridge.count("content: encodeBase64(snapshot)") == 2
