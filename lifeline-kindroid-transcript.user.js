...
(function lifelineTranscriptBridge() {
  ...
  function findTranscriptOption() {
    return [...document.querySelectorAll("[role='option'], [role='menuitem']")].find((option) => {
  ...
  function transcriptPanelIsOpen() {
    const panel = document.querySelector('[class*="call-transcript-panel-v2_"], [class*="call-transcript-panel_"]');
    return visible(panel);
  }

  async function activateTranscriptPanelOnce(callId) {
    if (!callId || transcriptPanelAttemptedForCall === callId || transcriptPanelIsOpen()) return;
    // Automatic captures may run every few seconds. Mark the call before any
    // interaction so later captures only read the DOM, even when activation fails.
  ...
  async function extractTranscript() {
    ...
    if (!rows.length) await activateTranscriptPanelOnce(callId);
    if (!rows.length) {
