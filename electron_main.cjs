...
function extractionScript() {
  return `(() => new Promise(async (resolve) => {
    ...
    const selectors = ['p.chat-bubble-v2_bubble-text__QUcZ9', 'p[class*="chat-bubble-v2_bubble-text"]', 'p.v2-message-selectable'];
    const findBubbles = () => {
      for (const selector of selectors) {
        const nodes = [...document.querySelectorAll(selector)].filter((node) => clean(node.innerText || node.textContent));
...
async function waitForBubbles(win) {
  for (let i = 0; i < 30; i += 1) {
    const count = await win.webContents.executeJavaScript(`document.querySelectorAll('p.chat-bubble-v2_bubble-text__QUcZ9, p[class*="chat-bubble-v2_bubble-text"], p.v2-message-selectable').length`, true);
...
ipcMain.handle('lifeline:fetch-group-transcript', async (_event, payload = {}) => {
  ...
  try {
    ...
    if (win.webContents.getURL() !== sourceUrl) await win.loadURL(sourceUrl);
