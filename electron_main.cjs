...
function getTranscriptWindow(groupId) {
  ...
  const win = new BrowserWindow({ width: 1280, height: 900, title: `Kindroid transcript ${groupId}`, show: true, webPreferences: { contextIsolation: true, nodeIntegration: false, session: session.defaultSession } });
...
async function navigateTranscriptWindow(win, sourceUrl, groupId) {
  if (isExpectedGroupPage(win.webContents.getURL(), groupId) && !win.webContents.isLoading()) return;
  let lastError;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      // Kindroid's client-side redirects can make Electron reject loadURL with
      // ERR_FAILED even though the requested route subsequently commits.
