...
function kindroidWebPreferences() {
  return {
    ...
    preload: path.join(APP_ROOT, 'kindroid_userscripts_preload.cjs'),
  };
}
...
ipcMain.handle('lifeline:get-kindroid-userscripts', (event) => {
  return kindroidUserscriptsFor(event.senderFrame.url);
});
