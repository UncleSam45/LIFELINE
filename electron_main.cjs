...
async function loadKindroidPanel(panel) {
  ...
  // only navigate when this is a new/uninitialized panel.
  const currentUrl = panel.webContents.getURL();
  if (!/^https:\/\/(?:www\.)?kindroid\.ai\//i.test(currentUrl)) await panel.loadURL(KINDROID_HOME_URL);
}
