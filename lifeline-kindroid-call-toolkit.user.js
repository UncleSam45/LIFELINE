...
(function createLifelineKindroidToolkit(global) {
  ...
  const CONTINUATION_DELAY_MS = 40000;
  let sendLockUntil = 0;
  ...
  function mount() {
    ...
    let monitorEnabled = false; let activeSpeaker = ''; let speakerSince = 0; let monitorTimer = null;
    ...
    const updateMonitor = () => {
      ...
      send('*CONTINUES CONVERSATION*').then(() => {
        ...
        monitorDetail.textContent = `${activeSpeaker} · timer restarted (40s)`;
      }).catch((error) => {
        showStatus(`Monitor: ${error.message}`, 'error');
        monitorDetail.textContent = `${activeSpeaker} · retrying in 40s`;
      });
    };
    const render = () => {
