...
def test_journal_navigation_goes_directly_to_the_working_journal_url():
    journal_navigation = ADAPTER.split("async openJournalPage", 1)[1].split("async selectJournalScope", 1)[0]
    assert "this.window.loadURL(target)" in journal_navigation
    assert "location.assign" not in journal_navigation
    assert "for(let attempt=0;attempt<60;attempt++)" in journal_navigation
    assert "await this.pageIsUsable(id)" in journal_navigation
...
def test_isolated_journal_window_is_visible_during_live_scan():
    ...
    assert "openDevTools({ mode:'detach', activate:true })" in preparation
