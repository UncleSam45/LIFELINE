...
class KindroidJournalAdapter {
  ...
  async openJournalPage(aiId) {
    ...
    for(let attempt=0;attempt<60;attempt++){await sleep(250);const url=this.window.webContents.getURL();if(/login|signin|auth/i.test(url))throw new Error('LOGIN REQUIRED');if(await this.pageIsUsable(expectedPath))return;}
