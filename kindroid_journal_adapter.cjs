...
class KindroidJournalAdapter {
  ...
  async openJournalPage(aiId) {
    ...
    const expectedPath=`/v2/kin-settings/${encodeURIComponent(id)}/`;const target=`https://kindroid.ai${expectedPath}?tab=journal`;let navigationError=null;
    for(let attempt=0;attempt<2;attempt++){try{await this.window.loadURL(target);navigationError=null;break;}catch(error){navigationError=error;await sleep(700);if(await this.pageIsUsable(expectedPath)){navigationError=null;break;}}}
    if(navigationError)throw new Error(`JOURNAL PAGE FAILED TO LOAD: ${navigationError.message}`);await sleep(1200);const url=this.window.webContents.getURL();if(/login|signin|auth/i.test(url))throw new Error('LOGIN REQUIRED');if(!await this.pageIsUsable(expectedPath))throw new Error('JOURNAL PAGE FAILED TO LOAD');
  }
  async selectJournalScope(scope) { const wanted=scope==='global'?'Global':'Personal'; const ok=await this.run(`() => { const norm=s=>String(s||'').replace(/\\s+/g,' ').trim(); const button=[...document.querySelectorAll('button[role="radio"]')].find(x=>norm(x.textContent)===${JSON.stringify(wanted)}); if(!button)return false;if(button.getAttribute('aria-checked')!=='true')button.click();return true; }`); if(!ok)throw new Error(scope==='global'?'GLOBAL SCOPE COULD NOT BE SELECTED':UI_MAP_ERROR); await sleep(900); const selected=await this.run(`() => [...document.querySelectorAll('button[role="radio"]')].some(x=>String(x.textContent||'').trim()===${JSON.stringify(wanted)}&&x.getAttribute('aria-checked')==='true')`); if(!selected)throw new Error(UI_MAP_ERROR); }
