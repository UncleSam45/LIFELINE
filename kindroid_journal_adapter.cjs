...
class KindroidJournalAdapter {
  ...
  async pageIsUsable(aiId='') { try { const url=new URL(this.window.webContents.getURL());if(!/(^|\.)kindroid\.ai$/i.test(url.hostname)||!url.pathname.includes(`/kin-settings/${encodeURIComponent(aiId)}`))return false;return await this.run(`() => document.readyState !== 'loading' && Boolean(document.body)`); } catch (_error) { return false; } }
  async openJournalPage(aiId) {
    const id=String(aiId||'').trim();if(!/^[A-Za-z0-9_-]{6,128}$/.test(id))throw new Error('NO VALID DIRECTORY AI ID');
    const target=`https://kindroid.ai/v2/kin-settings/${encodeURIComponent(id)}/?tab=journal`;
    // This is an isolated automation window, so navigate directly to the same
    // URL that worked before journal automation was separated from the panel.
    // A rejected promise is tolerated only when the expected page still renders.
    let navigationError=null;try{await this.window.loadURL(target);}catch(error){navigationError=error;}
    for(let attempt=0;attempt<60;attempt++){await sleep(250);const url=this.window.webContents.getURL();if(/login|signin|auth/i.test(url))throw new Error('LOGIN REQUIRED');if(await this.pageIsUsable(id)){await sleep(1200);return;}}
    if(navigationError)throw new Error(`JOURNAL PAGE FAILED TO LOAD: ${navigationError.message}`);
