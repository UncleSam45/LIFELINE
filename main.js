...
function renderGroupmakerReconnectButton() {
  ...
  const title = session?.group_id
    ...
    : ready
      ...
      : 'Open GROUPMAKER setup';
  return `<button id="gm-reconnect" class="gm-reconnect ${ready ? 'ready' : 'needs-setup'}" title="${escapeHtml(title)}"><span>↻</span><b>RECONNECT</b><small>${escapeHtml(session?.group_id ? 'Open saved Kindroid group' : ready ? 'Create Kindroid group' : 'No saved group yet')}</small></button>`;
}
