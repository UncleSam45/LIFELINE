...
async function updateFamilyMemory(entry) {
  ...
  const existingMemory = String(entry.ai_memory || '');
  const previousGenerated = String(entry.family_memory_generated || '');
  if (previousGenerated && existingMemory.includes(previousGenerated)) {
    entry.ai_memory = existingMemory.replace(previousGenerated, generated.text);
  } else if (!existingMemory) {
    entry.ai_memory = generated.text;
  } else {
    entry.ai_memory = `${existingMemory}${existingMemory.endsWith('\n') ? '' : '\n'}${generated.text}`;
  }
  entry.family_memory_generated = generated.text;
  persistLocalConfig(`Update generated family memory: ${entry.name || entry.directory_uid}`);
  const memoryField = document.querySelector('[data-field="ai_memory"]');
  if (memoryField) memoryField.value = entry.ai_memory;
