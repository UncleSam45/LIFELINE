...
async function runGroupmakerPresenceAutomations({ people = [], groupId = '', active = null, aiList = [] } = {}) {
  ...
  if (!location || !people.length) return { directSent: 0, groupSent: false, changedNames: [] };
  ...
  for (const person of people) {
    ...
    const result = await kindroidApiClient.execute('send_message', { ai_id: entry.ai_id, message, stream: false });
    if (!result.ok) throw new Error(`Location notice failed for ${entry.name || person.name || entry.ai_id}: ${result.message || result.status || 'Kindroid request failed'}`);
    entry.location = location;
    changedNames.push(entry.name || person.name || entry.ai_id);
    directSent += 1;
  }
  ...
  if (groupId && groupmakerPresenceChanged(active, aiList, location)) {
    ...
    if (names) {
      ...
      const result = await kindroidApiClient.execute('group_user_message', { group_id: groupId, message });
      if (!result.ok) throw new Error(`Group presence recap failed: ${result.message || result.status || 'Kindroid request failed'}`);
      groupSent = true;
    }
  }
  return { directSent, groupSent, changedNames };
}
