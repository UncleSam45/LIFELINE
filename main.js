...
function generationOptions(selectedIds = [], selfId = '') {
  ...
  return generationPeople().filter((person) => String(person.id || '').trim() && String(person.id).trim() !== selfId).sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''))).map((person) => {
    ...
    return `<label class="relation-choice"><input type="checkbox" value="${escapeHtml(id)}" ${selected.has(id) ? 'checked' : ''}/><span>${escapeHtml(label)}</span><small>${escapeHtml(id)}</small></label>`;
...
function relationList(ids) {
  return ids.length ? ids.map((id) => `<li><b>${escapeHtml(describeGenerationPerson(id))}</b><small>${escapeHtml(id)}</small></li>`).join('') : '<li class="muted">None linked yet.</li>';
}
...
function renderGenerationsSection(entry) {
  ...
  return `<section id="generations-section" class="generations-card tab-panel ${state.activeEntryTab === 'family' ? 'active' : ''}"><div><p class="eyebrow">FAMILY MAP</p><h3>Relationships & household</h3><p class="sync-note">Centralized family connections saved in config.json as generations_people and linked to this DIRECTORY person.</p></div>${generationTreeMarkup({ ...person, parents, children })}<div class="generation-summary"><div><b>${parents.length}</b><span>Parents</span></div><div><b>${children.length}</b><span>Children</span></div><div><b>${escapeHtml(person.status || '—')}</b><span>Status</span></div></div><form id="generation-form" class="field-grid generation-form"><label><span>GENERATION ID</span><input value="${escapeHtml(person.id)}" readonly /></label><label><span>STATUS</span><input data-generation-field="status" value="${escapeHtml(person.status || '')}" placeholder="Alive, Elder, Deceased…" /></label><label><span>SEX</span><input data-generation-field="sex" value="${escapeHtml(person.sex || '')}" /></label><label class="wide"><span>NOTES</span><textarea data-generation-field="notes">${escapeHtml(person.notes || '')}</textarea></label></form><div class="relations-grid"><div><h4>Parents & ancestry</h4><ul>${relationList(parents)}</ul><div id="generation-parents" class="relation-picker">${generationOptions(parents, person.id)}</div></div><div><h4>Children & descendants</h4><ul>${relationList(children)}</ul><div id="generation-children" class="relation-picker">${generationOptions(children, person.id)}</div></div></div></section>`;
}
...
function bindDirectoryEvents() {
  ...
  document.querySelector('#reload-bridge')?.addEventListener('click', loadBridge);
