...
const state = {
  ...
  journalSync: { open:false, busy:false, scope:'', aiId:'', personUid:'', plan:null, progress:'', error:'' },
};
...
function ensureJournalEntry(record = {}) {
  ...
  const normalized = {
    ...
    attempt_count: Math.max(0, Number(record.attempt_count || 0)), archived: Boolean(record.archived), deleted_at: record.deleted_at || null,
    kindroid_sync: { scope:'personal',remote_id:'',status:'never_synced',baseline_keywords:[],baseline_description:'',baseline_hash:'',remote_last_seen_hash:'',last_scanned_at:'',last_synced_at:'',last_operation:'',last_error:'',...(record.kindroid_sync||{}) },
  };
...
function journalsForPerson(person) {
  return journalEntries().filter((journal) => journal.directory_uid === person?.directory_uid && !journal.deleted_at).sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)));
}
...
function wikiEntries() {
  ...
  state.config.wiki_entries = state.config.wiki_entries.filter((entry) => entry && typeof entry === 'object').map(ensureWikiEntry);
  return state.config.wiki_entries;
}

function ensureWikiEntry(entry={}) { entry.journal_keywords=normalizeJournalKeyphrases(entry.journal_keywords?.length?entry.journal_keywords:[entry.word||'Untitled']);entry.journal_description=String(entry.journal_description ?? entry.content ?? '').slice(0,500);entry.deleted_at=entry.deleted_at||null;entry.kindroid_sync={scope:'global',remote_id:'',remote_access_ai_id:'',status:'never_synced',baseline_keywords:[],baseline_description:'',baseline_hash:'',remote_last_seen_hash:'',last_scanned_at:'',last_synced_at:'',last_operation:'',last_error:'',...(entry.kindroid_sync||{})};return entry;}
function ensureJournalSyncConfig(){const current=state.config.kindroid_journal_sync||{};state.config.schema_version=Math.max(2,Number(state.config.schema_version||0));state.config.kindroid_journal_sync={version:1,global:{initialized:false,access_ai_id:'',last_scan_at:'',last_sync_at:'',...(current.global||{})},personal:{...(current.personal||{})},tombstones:Array.isArray(current.tombstones)?current.tombstones:[],audit_log:Array.isArray(current.audit_log)?current.audit_log:[],pending_operation:current.pending_operation||null};return state.config.kindroid_journal_sync;}
...
async function writeBridgeConfig(reason, retryOnShaMismatch = true) {
  ...
  try {
    ...
    const localWiki = wikiEntries().map((entry) => ({ ...entry }));
    const localJournalSync = structuredClone(ensureJournalSyncConfig());
    ...
    const wikiMerged = new Map((latest.config.wiki_entries || []).map((entry) => [String(entry.id), entry]));
    localWiki.forEach((entry) => { const remote=wikiMerged.get(String(entry.id));if(!remote||String(entry.updated_at)>=String(remote.updated_at))wikiMerged.set(String(entry.id),entry); });
    const remoteSync=latest.config.kindroid_journal_sync||{};
    const uniqueAudit=[...(remoteSync.audit_log||[]),...(localJournalSync.audit_log||[])].filter((row,index,all)=>all.findIndex(other=>JSON.stringify(other)===JSON.stringify(row))===index);
    const uniqueTombstones=[...(remoteSync.tombstones||[]),...(localJournalSync.tombstones||[])].filter((row,index,all)=>all.findIndex(other=>String(other.id)===String(row.id)&&other.scope===row.scope)===index);
    state.config = { ...latest.config, ...state.config, journal_entries: [...merged.values()], wiki_entries:[...wikiMerged.values()], kindroid_journal_sync:{...remoteSync,...localJournalSync,personal:{...(remoteSync.personal||{}),...(localJournalSync.personal||{})},tombstones:uniqueTombstones,audit_log:uniqueAudit} };
    const payload = await githubRequest(bridgeUrl(BRIDGE_PATH, false), {
      method: 'PUT',
      body: JSON.stringify({
        message: `${reason} via LIFELINE frontend`,
...
function renderWikiView() {
  ...
  const all = wikiEntries().filter((entry) => !entry.deleted_at).sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')));
...
function journalLocalValue(record,scope){return scope==='global'?{keywords:record.journal_keywords,description:record.journal_description}:{keywords:record.keyphrases,description:record.entry};}
function journalScopeState(scope,personUid=''){const config=ensureJournalSyncConfig();if(scope==='global')return config.global;if(!config.personal[personUid])config.personal[personUid]={initialized:false,ai_id:'',last_scan_at:'',last_sync_at:''};return config.personal[personUid];}
function chooseGlobalAiId(){const saved=ensureJournalSyncConfig().global.access_ai_id;if(saved)return saved;const current=selectedEntry();return String(current?.ai_id||entries().map(ensureEntry).find(x=>x.online&&x.ai_id)?.ai_id||entries().map(ensureEntry).find(x=>x.ai_id)?.ai_id||'');}
function boundedJournalIpc(request, milliseconds, timeoutMessage){let timer;const timeout=new Promise((_resolve,reject)=>{timer=setTimeout(()=>reject(new Error(timeoutMessage)),milliseconds);});return Promise.race([request,timeout]).finally(()=>clearTimeout(timer));}
async function startJournalSync(scope,person=selectedEntry()){
  const sync=state.journalSync;const aiId=scope==='global'?chooseGlobalAiId():String(person?.ai_id||'');sync.open=true;sync.busy=true;sync.scope=scope;sync.aiId=aiId;sync.personUid=person?.directory_uid||'';sync.plan=null;sync.error='';sync.progress='Opening Kindroid and reading every journal entry…';render();
  try {
    if(!window.lifelineElectron?.journalSync)throw new Error('ELECTRON MODE REQUIRED — journal synchronization is available in the LIFELINE desktop app.');if(!aiId)throw new Error('NO VALID DIRECTORY AI ID');
    if(typeof window.lifelineElectron.journalSync.open!=='function')throw new Error('The Electron preload bridge is outdated. Restart LIFELINE before synchronizing journals.');
    sync.progress='Opening Kindroid journal…';render();
    const opened=await boundedJournalIpc(window.lifelineElectron.journalSync.open({scope,aiId}),35000,'Electron did not respond while opening Kindroid. Restart LIFELINE and retry.');
    if(!opened?.ok)throw new Error(`${opened?.stage?`${opened.stage}: `:''}${opened?.error||opened?.message||'Kindroid journal could not be opened.'}`);
    sync.progress='Kindroid journal opened. Selecting Global journals…';render();
    const result=await boundedJournalIpc(window.lifelineElectron.journalSync.scan({scope,aiId}),90000,'Journal scan timed out after 90 seconds. Kindroid may still be loading; retry the scan.');
    if(!result||typeof result!=='object')throw new Error('The Electron journal scanner returned no result.');
    if(!result.ok)throw new Error(`${result.stage?`${result.stage}: `:''}${result.error||result.message||'Journal scan failed.'}`);
    if(!Array.isArray(result.entries))throw new Error('The Electron journal scanner returned an invalid entries payload.');
    if(result.diagnostics?.failedCount)throw new Error(`${result.diagnostics.failedCount} journal entr${result.diagnostics.failedCount===1?'y':'ies'} could not be extracted; synchronization was stopped to protect remote data.`);
    sync.progress=`Found ${result.diagnostics?.rowCount??result.entries.length} journal rows. Building synchronization comparison…`;render();
    const scopeState=journalScopeState(scope,sync.personUid);scopeState.last_scan_at=new Date().toISOString();if(scope==='global')scopeState.access_ai_id=aiId;else scopeState.ai_id=aiId;
    const locals=(scope==='global'?wikiEntries():journalEntries().filter(x=>x.directory_uid===person?.directory_uid)).filter(x=>!x.archived||x.deleted_at||scope==='global');sync.plan=window.LifelineJournalSync.buildSynchronizationPlan({locals,remotes:result.entries,scope,initialized:scopeState.initialized});sync.plan.entriesScanned=result.entries.length;sync.progress='Scan complete. Review every action before applying.';
  } catch(error) { sync.error=error.message;sync.progress='Journal scan stopped safely.'; }
  finally { sync.busy=false;render(); }
}
function renderJournalSyncModal(){const sync=state.journalSync;if(!sync.open)return'';const plan=sync.plan;const label=sync.scope==='global'?'Global Wiki':directoryPersonByUid(sync.personUid)?.name||'Personal journal';const rows=plan?.operations.map((op,i)=>`<article class="journal-sync-row"><b>${escapeHtml(op.local?.word||op.local?.keyphrases?.[0]||op.remote?.keywords?.[0]||'Unmatched entry')}</b><span>${escapeHtml(op.action.replaceAll('_',' ').toUpperCase())}</span><details><summary>Compare versions</summary><pre>LOCAL\n${escapeHtml(JSON.stringify(op.local?journalLocalValue(op.local,sync.scope):null,null,2))}\n\nKINDROID\n${escapeHtml(JSON.stringify(op.remote||null,null,2))}</pre></details>${['conflict','uncertain_match'].includes(op.action)?`<select data-journal-resolution="${i}"><option value="">Choose resolution…</option><option value="local">Keep local version</option><option value="remote">Keep Kindroid version</option><option value="exclude">Cancel and edit manually</option></select>`:''}</article>`).join('')||'';const counts=plan?Object.entries(plan.counts).map(([k,v])=>`<span><b>${v}</b> ${escapeHtml(k.replaceAll('_',' '))}</span>`).join(''):'';return `<div class="kindroid-sync-backdrop"><section class="kindroid-sync-modal journal-sync-modal" role="dialog" aria-modal="true"><header><div><p class="eyebrow">JOURNAL RECONCILIATION</p><h2>${escapeHtml(label)}</h2><p>${escapeHtml(sync.scope)} · Access AI ID ${escapeHtml(sync.aiId||'—')}</p></div></header><p>${escapeHtml(sync.progress)}</p>${sync.error?`<div class="kindroid-sync-error">${escapeHtml(sync.error)}</div>`:''}${plan?`<div class="journal-sync-summary"><span>Scanned <b>${plan.entriesScanned}</b></span>${counts}</div><div class="journal-sync-rows">${rows}</div>`:''}<footer><button id="journal-sync-cancel" class="ghost">CANCEL</button>${plan?'<button id="journal-sync-refetch" class="ghost">REFETCH</button><button id="journal-sync-apply">APPLY SYNCHRONIZATION</button>':''}</footer></section></div>`;}

async function applyJournalSynchronization(){const sync=state.journalSync,plan=sync.plan;if(!plan||sync.busy)return;document.querySelectorAll('[data-journal-resolution]').forEach(select=>{if(select.value){const i=Number(select.dataset.journalResolution);plan.operations[i]=window.LifelineJournalSync.resolveConflict(plan.operations[i],select.value);}});if(plan.operations.some(x=>['conflict','uncertain_match'].includes(x.action))){sync.error='Resolve or exclude every conflict before applying.';render();return;}sync.busy=true;sync.error='';const cfg=ensureJournalSyncConfig();const scopeState=journalScopeState(sync.scope,sync.personUid);await saveBridge('Checkpoint journal synchronization plan',true);
  try{for(let index=0;index<plan.operations.length;index++){const op=plan.operations[index],now=new Date().toISOString();sync.progress=`${index+1}/${plan.operations.length}: ${op.action.replaceAll('_',' ')}`;render();cfg.pending_operation={scope:sync.scope,index,action:op.action,started_at:now};await saveBridge('Checkpoint pending journal operation',true);
      if(op.action==='import_remote'){const r=op.remote;if(sync.scope==='global'){op.local=ensureWikiEntry({id:newWikiId(),word:r.keywords[0]||r.title||'Untitled',content:r.description,journal_keywords:r.keywords,journal_description:r.description,created_at:now,updated_at:now});wikiEntries().push(op.local);}else{op.local=ensureJournalEntry({id:newJournalId(),directory_uid:sync.personUid,ai_id:sync.aiId,person_name:directoryPersonByUid(sync.personUid)?.name,entry:r.description,keyphrases:r.keywords,created_at:now,updated_at:now,remote_status:'synced'});journalEntries().push(op.local);}}
      else if(['create_remote','update_remote','delete_remote'].includes(op.action)){const operation=op.action==='create_remote'?'create':op.action==='update_remote'?'update':'delete';const value=op.local?journalLocalValue(op.local,sync.scope):{};const result=await window.lifelineElectron.journalSync.apply({scope:sync.scope,aiId:sync.aiId,operation,journal:value,remoteHandle:op.remote?.remote_handle});if(!result.ok)throw new Error(result.message);}
      else if(op.action==='update_local'){const r=op.remote;if(sync.scope==='global'){op.local.journal_keywords=r.keywords;op.local.journal_description=r.description;}else{op.local.keyphrases=r.keywords;op.local.entry=r.description;}op.local.updated_at=now;}
      else if(op.action==='delete_local'){op.local.deleted_at=now;}else if(op.action==='close_deletion'&&op.local){cfg.tombstones.push({id:op.local.id,scope:sync.scope,deleted_at:now});if(sync.scope==='global')state.config.wiki_entries=wikiEntries().filter(x=>x.id!==op.local.id);else state.config.journal_entries=journalEntries().filter(x=>x.id!==op.local.id);}
      const target=op.local;if(target&&!target.deleted_at&&op.action!=='delete_local'){const value=journalLocalValue(target,sync.scope);target.kindroid_sync.baseline_keywords=value.keywords;target.kindroid_sync.baseline_description=value.description;target.kindroid_sync.baseline_hash=window.LifelineJournalSync.hashJournal(value,sync.scope);target.kindroid_sync.last_synced_at=now;target.kindroid_sync.status='synced';}cfg.pending_operation=null;cfg.audit_log.push({scope:sync.scope,action:op.action,completed_at:now});await saveBridge('Verified journal synchronization operation',true);}
    sync.progress='Rescanning Kindroid to verify every operation…';render();
    const verification=await window.lifelineElectron.journalSync.scan({scope:sync.scope,aiId:sync.aiId});
    if(!verification?.ok||!Array.isArray(verification.entries))throw new Error(`Post-apply verification scan failed: ${verification?.error||verification?.message||'invalid scanner response'}`);
    if(verification.diagnostics?.failedCount)throw new Error(`Post-apply verification could not extract ${verification.diagnostics.failedCount} remote entries.`);
    const verificationLocals=(sync.scope==='global'?wikiEntries():journalEntries().filter(x=>x.directory_uid===sync.personUid)).filter(x=>!x.archived||x.deleted_at||sync.scope==='global');
    const remaining=window.LifelineJournalSync.buildSynchronizationPlan({locals:verificationLocals,remotes:verification.entries,scope:sync.scope,initialized:true});
    const unresolved=['create_remote','update_remote','delete_remote','conflict','uncertain_match'].reduce((total,key)=>total+(remaining.counts[key]||0),0);
    if(unresolved)throw new Error(`Post-apply verification found ${unresolved} unresolved synchronization operation(s).`);
    scopeState.initialized=true;scopeState.last_sync_at=new Date().toISOString();cfg.pending_operation=null;sync.progress=`Global journal synchronization complete. Remote entries: ${verification.entries.length}. Verification: Passed.`;await saveBridge('Complete verified journal synchronization',true);sync.plan=remaining;
  }catch(error){sync.error=error.message;sync.progress='Synchronization stopped safely.';}finally{sync.busy=false;render();}}
...
function bindDirectoryEvents() {
  ...
  document.querySelector('#wiki-sync')?.addEventListener('click', () => startJournalSync('global'));
  document.querySelector('#journal-sync')?.addEventListener('click', () => startJournalSync('personal', selectedEntry()));
  document.querySelector('#journal-sync-cancel')?.addEventListener('click', () => { if(!state.journalSync.busy){state.journalSync.open=false;render();} });
  document.querySelector('#journal-sync-refetch')?.addEventListener('click', () => startJournalSync(state.journalSync.scope,directoryPersonByUid(state.journalSync.personUid)));
  document.querySelector('#journal-sync-apply')?.addEventListener('click', applyJournalSynchronization);
  ...
  document.querySelector('#wiki-save')?.addEventListener('click', () => {
    ...
    entry.word = document.querySelector('#wiki-word')?.value.trim() || 'Untitled'; entry.content = document.querySelector('#wiki-content')?.value || ''; entry.journal_keywords=normalizeJournalKeyphrases(document.querySelector('#wiki-journal-keywords')?.value||entry.word); entry.journal_description=(document.querySelector('#wiki-journal-description')?.value||'').slice(0,500); entry.updated_at = new Date().toISOString(); saveBridge(`Update wiki entry: ${entry.word}`);
  });
  document.querySelector('#wiki-delete')?.addEventListener('click', () => {
    ...
    entry.deleted_at=new Date().toISOString();entry.kindroid_sync.status='pending_remote_delete'; state.selectedWikiId = state.config.wiki_entries[0]?.id || ''; saveBridge('Delete wiki entry');
  });
  ...
  document.querySelectorAll('.journal-delete').forEach((button) => button.addEventListener('click', () => { const record=journalEntries().find(row=>row.id===button.dataset.journalId);if(record&&confirm('Delete this journal locally? Remote deletion will be proposed during the next synchronization.')){record.deleted_at=new Date().toISOString();record.kindroid_sync.status='pending_remote_delete';record.updated_at=record.deleted_at;saveBridge('Soft delete personal journal');} }));
...
const JOURNAL_PROGRESS_LABELS={waiting_for_journal_page:'Waiting for journal page…',selecting_scope:'Selecting Global journals…',waiting_for_global_list:'Waiting for Global journal list…',rows_detected:'journal rows found',extracting_entry:'Extracting entry',verifying:'Verifying remote changes…',failed:'Journal synchronization failed.'};
window.lifelineElectron?.journalSync?.onProgress?.(({stage,detail})=>{const sync=state.journalSync;if(!sync.open||!sync.busy)return;const label=JOURNAL_PROGRESS_LABELS[stage]||stage;sync.progress=detail?`${label} ${detail}`:label;render();});
