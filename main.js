...
const LEGACY_TOKEN_STORAGE_KEYS = ['lifeline.accessKey', 'lifeline.github.accessKey', 'lifeline.github.token'];
const LEGACY_KINDROID_API_KEY_STORAGE_KEYS = ['kindroid.apiKey', 'lifeline.api.kindroidKey'];
function firstStoredValue(keys) {
  return keys.map((key) => localStorage.getItem(key) || sessionStorage.getItem(key) || '').find((value) => String(value || '').trim()) || '';
}
const REMEMBERED_ACCESS_KEY = firstStoredValue([TOKEN_STORAGE_KEY, ...LEGACY_TOKEN_STORAGE_KEYS]);
const REMEMBERED_KINDROID_API_KEY = firstStoredValue([KINDROID_API_KEY_STORAGE_KEY, ...LEGACY_KINDROID_API_KEY_STORAGE_KEYS]);
const REMEMBERED_KINDROID_CONNECTED = REMEMBERED_KINDROID_API_KEY.trim().startsWith('kn_');
const REMEMBERED_GITHUB_LOGIN_ENABLED = Boolean(REMEMBERED_ACCESS_KEY.trim());
...
const state = {
  ...
  rememberKey: localStorage.getItem(REMEMBER_STORAGE_KEY) === 'true' || Boolean(REMEMBERED_ACCESS_KEY.trim()),
  authenticated: false,
...
function hydrateStoredCredentials() {
  const storedAccessKey = firstStoredValue([TOKEN_STORAGE_KEY, ...LEGACY_TOKEN_STORAGE_KEYS]);
  const storedKindroidKey = firstStoredValue([KINDROID_API_KEY_STORAGE_KEY, ...LEGACY_KINDROID_API_KEY_STORAGE_KEYS]);
  if (storedAccessKey && !String(state.accessKey || '').trim()) state.accessKey = storedAccessKey;
  if (storedAccessKey) state.rememberKey = true;
  if (storedKindroidKey && !String(state.kindroidApiKey || '').trim()) state.kindroidApiKey = storedKindroidKey;
  state.kindroidConnected = String(state.kindroidApiKey || '').trim().startsWith('kn_');
}
...
function worldStatCards(all, activeSessions) {
  const online = all.filter((entry) => entry.online && !entry.archived).length;
  const locations = new Set(all.map((entry) => String(entry.location || '').trim()).filter(Boolean));
  return [
    ['Population', all.filter((entry) => !entry.archived).length, 'Active directory citizens'],
    ['Live Signals', online, 'Entities available right now'],
    ['Locations', locations.size, 'Known activity zones'],
    ['Sessions', activeSessions.length, 'Open GROUPMAKER rooms'],
  ];
}

function renderWorldView() {
  const all = entries().map(ensureEntry);
  const activePeople = all.filter((entry) => !entry.archived);
  const onlinePeople = activePeople.filter((entry) => entry.online);
  const activeSessions = groupmakerSessions().filter((session) => !String(session.closed_at || '').trim() && !String(session.idle_at || '').trim());
  const now = new Date();
  ...
  const weatherCards = [
    ['Local weather', 'Pending sensor link', 'Placeholder for live weather, alerts, sunrise, and commute conditions.'],
    ['World news', 'Briefing queue', 'Placeholder for trusted headlines and topics to share with virtual entities.'],
    ['Agenda', 'Today focus', 'Placeholder for calendar events, reminders, and daily priorities.'],
  ];
  const feed = [
    `World View initialized at ${timeLabel} on ${dayLabel}.`,
    `${onlinePeople.length} virtual ${onlinePeople.length === 1 ? 'entity is' : 'entities are'} marked live in DIRECTORY.`,
    activeSessions.length ? `${activeSessions.length} GROUPMAKER session(s) can be resumed.` : 'No open GROUPMAKER sessions detected.',
    'Real-world data connectors are staged as placeholders for the next build phase.',
  ];
  return `<section class="world-view" aria-label="WORLD VIEW home screen"><div class="world-orbit" aria-hidden="true"><span></span><span></span><span></span></div><div class="world-hero"><div><p class="eyebrow">WORLD VIEW</p><h1>WORLD VIEW</h1><p class="world-lede">Daily overview for real-world context, virtual population status, agenda items, weather, and news placeholders.</p><div class="world-time"><b>${escapeHtml(timeLabel)}</b><span>${escapeHtml(dayLabel)}</span></div></div><div class="world-globe" aria-hidden="true"><i></i><i></i><i></i></div></div><div class="world-stat-grid">${worldStatCards(activePeople, activeSessions).map(([label,value,note]) => `<article><span>${escapeHtml(label)}</span><b>${escapeHtml(value)}</b><small>${escapeHtml(note)}</small></article>`).join('')}</div><div class="world-grid"><section class="world-panel wide"><div class="panel-head"><p class="eyebrow">Daily Overview</p><h2>Briefing stack</h2></div><div class="signal-feed">${feed.map((item, index) => `<div class="signal-row"><span>0${index + 1}</span><p>${escapeHtml(item)}</p></div>`).join('')}</div></section>${weatherCards.map(([title,kicker,body]) => `<section class="world-panel"><div class="panel-head"><p class="eyebrow">${escapeHtml(kicker)}</p><h2>${escapeHtml(title)}</h2></div><p>${escapeHtml(body)}</p><button class="ghost world-placeholder" type="button">Connector planned</button></section>`).join('')}<section class="world-panel"><div class="panel-head"><p class="eyebrow">Virtual Population</p><h2>Who is awake?</h2></div><div class="world-roster">${onlinePeople.slice(0, 6).map((entry) => `<button class="world-person" data-world-uid="${escapeHtml(entry.directory_uid)}"><span class="avatar online">${entryInitials(entry)}</span><b>${escapeHtml(entry.name || 'Unnamed')}</b><small>${escapeHtml(entry.location || 'No location')}</small></button>`).join('') || '<div class="empty small">No virtual entities are online yet.</div>'}</div></section><section class="world-panel"><div class="panel-head"><p class="eyebrow">Data Fusion</p><h2>Future entity briefing</h2></div><p>These modules will eventually become structured context packets for AI entities: headlines, local conditions, calendar intent, user mood, and active world-state changes.</p><div class="world-tags"><span>News API</span><span>Weather API</span><span>Calendar</span><span>Memory feed</span></div></section></div></section>`;
}
...
function renderDirectory() {
  ...
  const sidebarContent = state.activeView === 'world' ? renderWorldSidebar() : renderDirectorySidebar(list);
  const editorContent = state.activeView === 'world' ? renderWorldView() : renderDirectoryEditor(current, onlineLabel);
  root.innerHTML = `<main class="app-shell"><aside class="sidebar"><div class="brand-block"><div class="brand-mark">LL</div><div><p class="eyebrow">LIFELINE</p><h2>Control Deck</h2></div></div><nav class="view-switch" aria-label="Primary view"><button id="world-view-toggle" class="ghost ${state.activeView === 'world' ? 'active' : ''}" type="button">WORLD VIEW</button><button id="directory-view-toggle" class="ghost ${state.activeView === 'directory' ? 'active' : ''}" type="button">DIRECTORY</button></nav><div class="sync-pill" title="Sync status"><span></span><b>${escapeHtml(state.syncState)}</b><small>${escapeHtml(state.syncDetail)}</small></div>${sidebarContent}${renderSettingsPanel()}</aside><section class="editor">${editorContent}</section>${renderKindroidApiStudio()}${renderGroupmakerWindow()}</main>`;
  bindDirectoryEvents();
}
...
function bindDirectoryEvents() {
  if (document.querySelector('#filter')) document.querySelector('#filter').value = state.filter;
  document.querySelector('#world-view-toggle')?.addEventListener('click', () => { state.activeView = 'world'; render(); });
  document.querySelector('#directory-view-toggle')?.addEventListener('click', () => { state.activeView = 'directory'; render(); });
  document.querySelector('#world-open-directory')?.addEventListener('click', () => { state.activeView = 'directory'; render(); });
  document.querySelectorAll('.world-person').forEach((button) => button.addEventListener('click', () => { state.selectedUid = button.dataset.worldUid; state.activeView = 'directory'; state.activeEntryTab = 'profile'; render(); }));
...
/* WORLD VIEW home deck */
.world-sidebar{display:grid;gap:.85rem;padding:.2rem}.world-sidebar h3{margin:.1rem 0;font-size:1.55rem;color:#eaf8ff;letter-spacing:-.04em}.world-side-grid{display:grid;grid-template-columns:1fr 1fr;gap:.55rem}.world-side-grid article{padding:.9rem;border:1px solid rgba(105,229,255,.14);border-radius:20px;background:rgba(105,229,255,.06);text-align:center}.world-side-grid b{display:block;font-size:1.8rem;color:#bdf5ff}.world-side-grid span{color:var(--muted);font-size:.68rem;text-transform:uppercase;letter-spacing:.11em}.credential-strip{display:grid;gap:.45rem;padding:.75rem;border:1px solid rgba(255,255,255,.1);border-radius:18px;background:rgba(255,255,255,.045);color:var(--muted);font-size:.78rem}.world-side-actions{display:grid;gap:.55rem}.world-side-actions button{text-align:center}
.view-switch{display:grid;grid-template-columns:1fr 1fr;gap:.45rem;padding:.35rem;border:1px solid rgba(255,255,255,.1);border-radius:20px;background:rgba(255,255,255,.045)}.view-switch button{padding:.75rem .65rem;border-radius:15px;letter-spacing:.1em;font-size:.72rem}.view-switch button.active{background:linear-gradient(135deg,rgba(105,229,255,.96),rgba(255,107,203,.92));color:#041020;box-shadow:0 14px 32px rgba(105,229,255,.22)}.world-view{position:relative;min-height:100%;isolation:isolate}.world-orbit{position:absolute;inset:0;pointer-events:none;overflow:hidden;border-radius:28px}.world-orbit span{position:absolute;border:1px solid rgba(105,229,255,.16);border-radius:999px;filter:drop-shadow(0 0 20px rgba(105,229,255,.2));animation:worldDrift 18s linear infinite}.world-orbit span:nth-child(1){width:42rem;height:42rem;right:-17rem;top:-15rem}.world-orbit span:nth-child(2){width:30rem;height:30rem;left:-13rem;bottom:8rem;animation-duration:24s}.world-orbit span:nth-child(3){width:18rem;height:18rem;right:18%;bottom:-9rem;animation-duration:14s}.world-hero{position:relative;display:grid;grid-template-columns:minmax(0,1fr) minmax(220px,32vw);gap:1.5rem;align-items:center;padding:clamp(1.2rem,3vw,2.4rem);border:1px solid rgba(105,229,255,.22);border-radius:34px;background:radial-gradient(circle at 78% 26%,rgba(105,229,255,.24),transparent 24rem),linear-gradient(135deg,rgba(10,18,45,.88),rgba(22,8,38,.78));box-shadow:0 28px 90px rgba(0,0,0,.34);overflow:hidden}.world-hero h1{max-width:900px;font-size:clamp(3rem,7vw,7.6rem);text-shadow:0 0 34px rgba(105,229,255,.2)}.world-lede{max-width:780px;color:#c9d8f2;font-size:1.08rem;line-height:1.8}.world-time{display:inline-grid;grid-template-columns:auto auto;gap:.65rem;align-items:end;margin-top:1rem;padding:.65rem .85rem;border:1px solid rgba(255,255,255,.12);border-radius:18px;background:rgba(255,255,255,.065)}.world-time b{font-size:2rem;color:#bdf5ff}.world-time span{color:var(--muted);padding-bottom:.28rem}.world-globe{justify-self:center;position:relative;width:min(28vw,330px);aspect-ratio:1;border-radius:999px;background:radial-gradient(circle at 35% 28%,#e8fbff,rgba(105,229,255,.7) 10%,rgba(139,92,246,.18) 38%,rgba(3,5,18,.18) 66%),repeating-linear-gradient(90deg,transparent 0 22px,rgba(255,255,255,.08) 23px 24px);box-shadow:inset -34px -30px 70px rgba(1,7,20,.82),0 0 70px rgba(105,229,255,.25);animation:globePulse 5s ease-in-out infinite}.world-globe i{position:absolute;inset:12%;border:1px solid rgba(255,255,255,.24);border-radius:999px;transform:rotateX(68deg)}.world-globe i:nth-child(2){inset:24%;transform:rotateY(66deg)}.world-globe i:nth-child(3){inset:36%;transform:rotate(45deg)}.world-stat-grid{position:relative;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1rem;margin:1rem 0}.world-stat-grid article,.world-panel{border:1px solid rgba(255,255,255,.11);border-radius:26px;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(105,229,255,.035));box-shadow:0 20px 60px rgba(0,0,0,.18);backdrop-filter:blur(18px)}.world-stat-grid article{padding:1rem}.world-stat-grid span,.world-stat-grid small{display:block;color:var(--muted);font-size:.72rem;letter-spacing:.11em;text-transform:uppercase}.world-stat-grid b{display:block;margin:.25rem 0;font-size:2.2rem;color:#f7fbff}.world-grid{position:relative;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem}.world-panel{padding:1rem;min-height:190px}.world-panel.wide{grid-column:span 2}.world-panel h2{margin:.15rem 0 .8rem;color:#eaf8ff;font-size:1.35rem}.world-panel p{color:#c6d5ed;line-height:1.65}.signal-feed{display:grid;gap:.65rem}.signal-row{display:grid;grid-template-columns:auto 1fr;gap:.75rem;align-items:center;padding:.75rem;border-radius:18px;background:rgba(3,8,20,.42);border:1px solid rgba(105,229,255,.12)}.signal-row span{color:#69e5ff;font-weight:950}.signal-row p{margin:0}.world-placeholder{margin-top:.75rem;box-shadow:none}.world-roster{display:grid;gap:.55rem}.world-person{display:grid;grid-template-columns:auto 1fr;gap:.1rem .65rem;align-items:center;text-align:left;background:rgba(84,246,166,.08);color:#eaf2ff;box-shadow:none}.world-person .avatar{grid-row:1/3}.world-person small{color:var(--muted)}.world-tags{display:flex;flex-wrap:wrap;gap:.45rem}.world-tags span{padding:.45rem .65rem;border-radius:999px;background:rgba(105,229,255,.1);border:1px solid rgba(105,229,255,.18);color:#dff7ff;font-size:.78rem}@keyframes worldDrift{to{transform:rotate(360deg)}}@keyframes globePulse{50%{transform:translateY(-8px) scale(1.02);filter:saturate(1.2)}}@media(max-width:1180px){.world-hero{grid-template-columns:1fr}.world-globe{width:min(55vw,300px)}.world-stat-grid,.world-grid{grid-template-columns:1fr 1fr}.world-panel.wide{grid-column:1/-1}}@media(max-width:720px){.world-stat-grid,.world-grid{grid-template-columns:1fr}.world-hero h1{font-size:3rem}.world-time{grid-template-columns:1fr}.world-globe{width:220px}}
