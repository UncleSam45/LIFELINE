...
const KINDROID_WEB_URL = 'https://kindroid.ai/v2/kins/';
...
const state = {
  ...
  settingsOpen: false,
  kindroidWebOpen: false,
  kindroidWebMinimized: false,
  kindroidWebDocked: false,
};
...
function renderKindroidWebWindow() {
  if (!state.kindroidWebOpen) return '';
  const modeClass = state.kindroidWebDocked ? 'docked' : 'floating';
  return `<section class="kindroid-web ${modeClass} ${state.kindroidWebMinimized ? 'mini' : ''}" aria-label="Kindroid website panel">
    <header class="kindroid-web-head">
      <div><p class="eyebrow">KINDROID WEB</p><h2>Kindroid</h2></div>
      <div class="kindroid-web-actions">
        <a class="kindroid-external" href="${KINDROID_WEB_URL}" target="_blank" rel="noopener noreferrer">Open ↗</a>
        <button id="kindroid-web-dock" class="ghost" type="button">${state.kindroidWebDocked ? 'Float' : 'Dock'}</button>
        <button id="kindroid-web-min" class="ghost" type="button" aria-label="${state.kindroidWebMinimized ? 'Restore' : 'Minimize'} Kindroid panel">${state.kindroidWebMinimized ? '□' : '—'}</button>
        <button id="kindroid-web-close" class="ghost" type="button" aria-label="Close Kindroid panel">×</button>
      </div>
    </header>
    ${state.kindroidWebMinimized ? '' : `<div class="kindroid-web-frame-wrap">
      <iframe src="${KINDROID_WEB_URL}" title="Kindroid kins" allow="clipboard-read; clipboard-write; microphone; camera; autoplay; fullscreen" referrerpolicy="strict-origin-when-cross-origin"></iframe>
      <p class="kindroid-web-help">If Kindroid stays blank or shows a blocked-page message, its security policy does not allow embedding. Use <b>Open ↗</b> to continue in a separate tab; LIFELINE cannot override that site policy.</p>
    </div>`}
  </section>`;
}
...
function bindDirectoryEvents() {
  ...
  document.querySelector('#kindroid-web-toggle')?.addEventListener('click', () => { state.kindroidWebOpen = !state.kindroidWebOpen; render(); });
  document.querySelector('#kindroid-web-close')?.addEventListener('click', () => { state.kindroidWebOpen = false; render(); });
  document.querySelector('#kindroid-web-min')?.addEventListener('click', () => { state.kindroidWebMinimized = !state.kindroidWebMinimized; render(); });
  document.querySelector('#kindroid-web-dock')?.addEventListener('click', () => { state.kindroidWebDocked = !state.kindroidWebDocked; state.kindroidWebMinimized = false; render(); });
...
.kindroid-web{z-index:30;border:1px solid rgba(105,229,255,.35);background:rgba(5,10,24,.97);box-shadow:0 30px 100px rgba(0,0,0,.68);overflow:hidden}.kindroid-web.floating{position:fixed;right:1.25rem;top:1.25rem;width:min(720px,calc(100vw - 2.5rem));height:min(820px,calc(100vh - 2.5rem));border-radius:28px}.kindroid-web.docked{grid-column:1/-1;height:min(820px,calc(100vh - 2rem));margin-top:1rem;border-radius:30px}.kindroid-web.mini{width:min(390px,calc(100vw - 2.5rem));height:auto}.kindroid-web-head{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.75rem 1rem;border-bottom:1px solid rgba(255,255,255,.1)}.kindroid-web-head h2{margin:0;font-size:1.25rem}.kindroid-web-head .eyebrow{margin:0 0 .15rem}.kindroid-web-actions{display:flex;align-items:center;gap:.4rem}.kindroid-web-actions button,.kindroid-external{padding:.58rem .72rem;border-radius:13px;font-size:.78rem}.kindroid-external{display:inline-block;background:linear-gradient(135deg,var(--cyan),var(--violet));color:#03101c;font-weight:900;text-decoration:none}.kindroid-web-frame-wrap{display:grid;grid-template-rows:minmax(0,1fr) auto;height:calc(100% - 68px)}.kindroid-web iframe{width:100%;height:100%;min-height:480px;border:0;background:#fff}.kindroid-web-help{margin:0;padding:.65rem 1rem;border-top:1px solid rgba(255,184,107,.25);background:rgba(255,184,107,.08);color:#ffd5a8;font-size:.76rem;line-height:1.45}@media(max-width:620px){.kindroid-web.floating{inset:.5rem;width:calc(100vw - 1rem);height:calc(100vh - 1rem)}.kindroid-web-head{align-items:flex-start}.kindroid-web-actions{flex-wrap:wrap;justify-content:flex-end}.kindroid-web iframe{min-height:360px}}
