...
function ensureGenerationPerson(entry) {
  ...
  person.rank = String(person.rank || entry.rank || '').trim();
...
function generationTreeMarkup(focus) {
  ...
  return `<div class="tree-board" style="--tree-rows:${rows.length}">${rows.map((row) => `<div class="tree-row">${row.map((person) => `<div class="tree-node ${person.id === focus.id ? 'focus' : ''}"><b>${escapeHtml(person.name || 'Unnamed')}</b><small>${escapeHtml([person.sex, person.rank].filter(Boolean).join(' • ') || person.id)}</small></div>`).join('')}</div>`).join('')}<div class="tree-edges">${edges.map(([from, to]) => `<span>${escapeHtml(describeGenerationPerson(from))} → ${escapeHtml(describeGenerationPerson(to))}</span>`).join('')}</div></div>`;
}
...
function renderGenerationsSection(entry) {
  ...
  return `<section id="generations-section" class="generations-card"><div><p class="eyebrow">GENERATIONS</p><h3>Relationships & family tree</h3><p class="sync-note">Mirrors the legacy GENERATIONS/TREE setup: this data is saved in config.json as generations_people and linked to the selected DIRECTORY person.</p></div>${generationTreeMarkup({ ...person, parents, children })}<div class="generation-summary"><div><b>${parents.length}</b><span>Parents</span></div><div><b>${children.length}</b><span>Children</span></div><div><b>${escapeHtml(person.status || '—')}</b><span>Status</span></div></div><form id="generation-form" class="field-grid generation-form"><label><span>GENERATION ID</span><input value="${escapeHtml(person.id)}" readonly /></label><label><span>STATUS</span><input data-generation-field="status" value="${escapeHtml(person.status || '')}" placeholder="Alive, Elder, Deceased…" /></label><label><span>SEX</span><input data-generation-field="sex" value="${escapeHtml(person.sex || '')}" /></label><label><span>RANK</span><input data-generation-field="rank" value="${escapeHtml(person.rank || '')}" /></label><label class="wide"><span>NOTES</span><textarea data-generation-field="notes">${escapeHtml(person.notes || '')}</textarea></label></form><div class="relations-grid"><div><h4>Parents</h4><ul>${relationList(parents)}</ul><div id="generation-parents" class="relation-picker">${generationOptions(parents, person.id)}</div></div><div><h4>Children</h4><ul>${relationList(children)}</ul><div id="generation-children" class="relation-picker">${generationOptions(children, person.id)}</div></div></div></section>`;
}
...
function groupmakerReconnectReady() {
  ...
  const hasActiveSession = Boolean(active?.group_id && Array.isArray(active.ai_list) && active.ai_list.length);
  return state.kindroidApiKey.startsWith('kn_') && (hasActiveSession || (state.groupmakerContext.trim() && people.length));
}
...
function renderGroupmakerReconnectButton() {
  ...
  const title = ready
    ? `${active ? 'Reopen/update' : 'Create'} Kindroid group from saved GROUPMAKER state`
    : 'Open GROUPMAKER to finish API key, names, and context setup';
  return `<button id="gm-reconnect" class="gm-reconnect ${ready ? 'ready' : 'needs-setup'}" title="${escapeHtml(title)}"><span>${ready ? '↻' : '⚙'}</span><b>${ready ? 'RECONNECT' : 'SETUP GROUPMAKER'}</b><small>${escapeHtml(active ? 'Use active Kindroid group' : ready ? 'Create Kindroid group' : 'Needs setup')}</small></button>`;
}
...
function bindDirectoryEvents() {
  ...
  document.querySelector('#generations-menu')?.addEventListener('click', () => document.querySelector('#generations-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  document.querySelector('#gm-reconnect')?.addEventListener('click', () => { if (groupmakerReconnectReady()) syncGroupmaker(); else { state.groupmakerOpen = true; state.groupmakerMinimized = false; state.groupmakerStatus = 'Finish GROUPMAKER setup: connect a Kindroid API key, add names, and add group context.'; render(); } });
  ...
  document.querySelectorAll('[data-generation-field]').forEach((input) => input.addEventListener('input', (e) => {
    ...
    if (e.target.dataset.generationField === 'rank') current.rank = e.target.value;
