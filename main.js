...
function familyMemoryForEntry(entry) {
  ...
  const formatNames = (names) => names.length === 1 ? names[0] : `${names.slice(0, -1).join(', ')} and ${names.at(-1)}`;
  const childOverview = children.length === 1
    ? `${firstName} has a ${childRole(children[0])} named ${String(children[0].name || 'Unnamed child').trim()}.`
    : children.length > 1
      ? `${firstName}'s children are ${formatNames(children.map((child) => `${String(child.name || 'Unnamed child').trim()} (${childRole(child)})`))}.`
      : '';
  const coParentSentences = [...childrenByPartner.entries()]
    .sort(([left], [right]) => String(byId.get(left)?.name || '').localeCompare(String(byId.get(right)?.name || '')))
    .map(([partnerId, partnerChildren]) => {
      const partnerName = conversationalName(byId.get(partnerId)?.name || 'Unknown partner', 'Unknown partner');
      const childNames = partnerChildren.map((child) => String(child.name || 'Unnamed child').trim());
      return `${firstName} shares ${formatNames(childNames)} with ${partnerName}.`;
    });
  const familySentences = [childOverview, ...coParentSentences].filter(Boolean);
