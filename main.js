...
const READ_ONLY_LEGACY_BRIDGE_PATHS = ['config.backup.json', 'kindroidxl_directory/config.json', 'kindroidxl_directory/config.backup.json', 'backups/config.json', 'backups/config.backup.json'];
...
function generationOptions(selectedIds = [], selfId = '') {
  ...
  const directoryCandidates = entries().map((entry) => ({ entry, person: findGenerationPersonForEntry(entry) })).filter(({ person }) => person && String(person.id || '').trim() !== selfId);
  const directoryIds = new Set(directoryCandidates.map(({ person }) => String(person.id || '').trim()));
  const candidates = [
    ...directoryCandidates,
    ...generationPeople().filter((person) => String(person.id || '').trim() !== selfId && !directoryIds.has(String(person.id || '').trim())).map((person) => ({ entry: null, person })),
  ];
  return candidates.sort((left, right) => String(left.person.name || '').localeCompare(String(right.person.name || ''))).map(({ entry, person }) => {
    const id = String(person.id).trim();
    const value = String(person.directory_uid || id).trim();
    const label = String(person.name || 'Unnamed').trim();
    const source = entry ? `Directory profile${String(entry.ai_id || '').trim() ? '' : ' · Local only'}` : 'Family-only profile';
    return `<label class="relation-choice" data-relation-name="${escapeHtml(label.toLowerCase())}"><input type="checkbox" value="${escapeHtml(value)}" ${selected.has(id) ? 'checked' : ''}/><span>${escapeHtml(label)}</span><small>${escapeHtml(source)}</small></label>`;
...
async function writeBridgeConfig(reason, retryOnShaMismatch = true) {
  const expectedDirectoryProfiles = entries().map((entry) => ({ uid: String(entry.directory_uid || '').trim(), name: String(entry.name || '').trim() })).filter((entry) => entry.uid);
  ...
  try {
    ...
    await verifyRootBridgeConfig(expectedDirectoryProfiles);
...
async function verifyRootBridgeConfig(expectedDirectoryProfiles) {
  const saved = await readGithubContentFile(BRIDGE_PATH);
  const savedProfiles = new Map((saved.config.directory_entries || []).map((entry) => [String(entry?.directory_uid || '').trim(), String(entry?.name || '').trim()]));
  const missing = expectedDirectoryProfiles.filter(({ uid, name }) => !savedProfiles.has(uid) || savedProfiles.get(uid) !== name);
  if (missing.length) throw new Error(`GitHub verification failed: ${missing.length} Directory profile(s) are missing from root ${BRIDGE_PATH}.`);
  state.bridgeSha = saved.sha || state.bridgeSha;
}
...
async function loadBridge() {
  ...
  if (!selectedCandidate && lastError && /not found/i.test(lastError.message)) for (const path of READ_ONLY_LEGACY_BRIDGE_PATHS) {
    try {
      const { sha, config } = await readGithubContentFile(path);
      selectedCandidate = { path, sha, config };
      break;
    } catch (error) {
      lastError = error;
  ...
  if (selectedCandidate) {
    state.bridgeSha = selectedCandidate.path === BRIDGE_PATH ? selectedCandidate.sha : '';
    ...
    state.syncDetail = selectedCandidate.path === BRIDGE_PATH
      ? `Restored ${entries().length} directory entries from ${BRIDGE_REPO}/${BRIDGE_PATH}.`
      : `Recovered ${entries().length} entries from read-only legacy path ${selectedCandidate.path}; next save will migrate them to root ${BRIDGE_PATH}.`;
