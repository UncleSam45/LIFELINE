...
async function startJournalSync(scope,person=selectedEntry()){
  ...
  try {
    ...
    let timer;const timeout=new Promise((_resolve,reject)=>{timer=setTimeout(()=>reject(new Error('Journal scan timed out after 90 seconds. Kindroid may still be loading; retry the scan.')),90000);});
    let result;try{result=await Promise.race([window.lifelineElectron.journalSync.scan({scope,aiId}),timeout]);}finally{clearTimeout(timer);}
