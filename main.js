...
async function runGroupmakerPresenceAutomations({ people = [], groupId = '', active = null, aiList = [] } = {}) {
  ...
  if (location && groupId && groupmakerPresenceChanged(active, aiList, location)) {
  ...
  for (const person of people) {
    ...
    if (activity) {
      const sceneResult = await kindroidApiClient.execute('update_info', { ai_id: entry.ai_id, current_scene: activity, __include_current_scene: 'set' }, { timeoutMs: 12000 });
      if (sceneResult.ok) {
...
async function syncGroupmaker() {
  ...
  const groupName = composeGroupName(people, active?.group_name || '');
  ...
  try {
    ...
    if (active) {
      ...
      Object.assign(target, { ai_list: aiList, names: sessionNames, group_name: groupName, group_context: context, group_directive: PHONE_CALL_DIRECTIVE, current_scene: activity, physical_location: groupmakerPhysicalLocation(), share_short_term_memory: true, use_manual_turntaking: true, touched_at: now, last_automation: { ...automation, ran_at: now } });
      const opened = openPreparedGroupmakerTab(preparedTab, target.group_id);
      state.groupmakerStatus = `Updated active session (${result.status}). Automation: ${automation.scenesUpdated}/${people.length} participant scene(s) updated; ${automation.groupSent ? 'group recap sent' : 'no group recap'}; ${automation.directSent} location notice(s) sent${automation.errors.length ? `; ${automation.errors.length} warning(s)` : ''}. ${opened ? 'Opened Kindroid call tab.' : `Open manually: ${kindroidGroupCallUrl(target.group_id)}`}`;
    } else {
      ...
      target = { session_key: groupId, group_id: groupId, ai_list: aiList, names: sessionNames, group_name: groupName, group_context: context, group_directive: PHONE_CALL_DIRECTIVE, current_scene: activity, physical_location: groupmakerPhysicalLocation(), share_short_term_memory: true, use_manual_turntaking: true, touched_at: now, closed_at: '', idle_at: '', last_automation: { ...automation, ran_at: now } };
