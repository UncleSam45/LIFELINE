...
const EXPERIMENTAL_KINDROID_API_REGISTRY = Object.freeze({
  ...
  directory_update_kin: op('directory_update_kin','Directory Compatibility Update','experimental','experimental_verified','POST','/update-info',legacyKinFields.map(x=>kField(x,x,/memory|story|directive|context|scene|description/.test(x)?'textarea':'text',{required:x==='ai_id',partial:x!=='ai_id',support:officialKinUpdate.includes(x)||x==='ai_id'?'official_equivalent':'experimental',officiallySupported:officialKinUpdate.includes(x)||x==='ai_id',sensitive:/memory|story|directive|context|scene|description/.test(x)})),{supportsPartialUpdates:true,sourceNotes:[legacySource('Recovered Directory update route; compatibility fields are warnings when they fail.')] }),
...
function freshDirectorySyncState(uid) { return { open:true, busy:false, action:'', personUid:uid, step:'Preparing', completedSteps:[], currentJournal:0, totalJournals:0, warnings:[], error:null, result:null, technical:null, retryBridgeSave:false }; }
function openDirectoryKindroidActionModal(directoryUid) { if (!directoryPersonByUid(directoryUid)) return; state.directoryKindroidSync = freshDirectorySyncState(directoryUid); render(); }
function closeDirectoryKindroidActionModal() { if (state.directoryKindroidSync.busy) return; state.directoryKindroidSync.open = false; render(); }
...
function buildDirectoryExperimentalUpdatePayload(person, aiId = person.ai_id) { return withDirectoryInclusionMarkers({ ai_id:aiId, user_set_temperature:person.temperature, reasoning_effort:person.reasoning_effort, llm_flair:person.llm_flair, ai_avatar:person.avatar_preset, custom_avatar_description:person.avatar_description }); }
...
async function pushDirectoryKinUpdate(person, aiId) { const official = await executeKindroidOperation('update_info', buildDirectoryOfficialUpdatePayload(person, aiId)); if (!official.ok) return { ok:false, official, experimental:null }; const experimentalPayload = buildDirectoryExperimentalUpdatePayload(person, aiId); const experimental = Object.keys(experimentalPayload).length > 1 ? await executeKindroidOperation('directory_update_kin', experimentalPayload) : null; return { ok:true, official, experimental, warning:experimental && !experimental.ok ? `Compatibility fields were not synchronized (${experimental.status || experimental.category}).` : '' }; }
...
async function updateDirectoryPersonOnKindroid(directoryUid) {
  ...
  try { let person=await prepareDirectoryRemoteAction(directoryUid,'update'); const aiId=String(person.ai_id); const journals=await replayDirectoryJournals(person,aiId,'update'); updateDirectorySyncProgress('Updating profile','Journal replay finished'); apiResult=await pushDirectoryKinUpdate(person,aiId); sync.technical=directoryTechnical(apiResult.official,'update_info'); if (!apiResult.ok) throw new Error(apiResult.official.message || 'Kindroid profile update failed.'); if (apiResult.warning) sync.warnings.push(apiResult.warning); if (journals.failures.length) sync.warnings.push(`${journals.failures.length} journal(s) remain failed or uncertain.`); person=directoryPersonByUid(directoryUid); person.online=true; person.kindroid_sync={ ...(person.kindroid_sync||{}), last_updated_at:new Date().toISOString() }; const result={ operation:'update', status:sync.warnings.length?'completed_with_warnings':'success', aiId, journalsAttempted:journals.attempted, journalsSynced:journals.synced, profileStatus:apiResult.official.status, warnings:sync.warnings, online:true }; recordDirectorySyncResult(person,result); sync.result=result; updateDirectorySyncProgress('Saving final state','Kindroid profile updated'); try { await queueBridgeSave('Finalize Directory Kindroid update'); updateDirectorySyncProgress(sync.warnings.length?'Completed with warnings':'Completed','Final bridge checkpoint'); } catch(error) { person.online=false; sync.retryBridgeSave=true; throw new Error(`Kindroid succeeded, but the final bridge save failed: ${error.message}`); }
  } catch(error) { const person=directoryPersonByUid(directoryUid); if(person) person.online=false; failDirectorySync(error,apiResult?.official || apiResult,'update_info'); } finally { sync.busy=false; activeDirectoryKindroidOperations.delete(directoryUid); render(); }
}
