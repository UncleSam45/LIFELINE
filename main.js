...
async function writeBridgeConfig(reason, retryOnShaMismatch = true) {
  ...
  try {
    ...
    await writeBridgeSnapshotArchive(snapshot);
...
async function writeBridgeSnapshotArchive(snapshot) {
  const stamp = String(snapshot?._lifeline_snapshot?.created_at || new Date().toISOString()).replace(/[^0-9]/g, '').slice(0, 17);
  const path = `snapshots/config_${stamp}_${SNAPSHOT_SESSION_ID}.json`;
  await githubRequest(bridgeUrl(path, false), {
    method: 'PUT',
    body: JSON.stringify({
...
async function loadBridge() {
  ...
  if (selectedCandidate) {
    state.bridgeSha = selectedCandidate.sha;
    state.config = selectedCandidate.config;
    entries().forEach(ensureEntry);
    syncGenerationPeopleWithDirectory();
    persistLocalConfig('Startup restore from root config.json');
    journalEntries();
    hydrateGroupmakerDraft();
    const dailyRollover = closeExpiredGroupmakerSessions();
    state.authenticated = true;
    state.syncState = 'Online';
    state.syncDetail = `Restored ${entries().length} directory entries from ${BRIDGE_REPO}/${BRIDGE_PATH}; local state is now authoritative for this session.`;
