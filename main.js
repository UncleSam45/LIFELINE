...
function generationOptions(selectedIds = [], selfId = '') {
  ...
  return generationPeople().filter((person) => String(person.id || '').trim() && String(person.id).trim() !== selfId).sort((a, b) => String(a.name || '').localeCompare(String(b.name || ''))).map((person) => {
    ...
    return `<label class="relation-choice"><input type="checkbox" value="${escapeHtml(value)}" ${selected.has(id) ? 'checked' : ''}/><span>${escapeHtml(label)}</span><small>${escapeHtml(person.status || person.sex || 'Family profile')}</small></label>`;
