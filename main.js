...
async function startJournalSync(scope,person=selectedEntry()){
  ...
  if(!window.lifelineElectron?.journalSync){sync.busy=false;sync.error='ELECTRON MODE REQUIRED — journal synchronization is available in the LIFELINE desktop app.';render();return;}if(!aiId){sync.busy=false;sync.error='NO VALID DIRECTORY AI ID';render();return;}
  const result=await window.lifelineElectron.journalSync.scan({scope,aiId});if(!result.ok){sync.busy=false;sync.error=result.message;render();return;}const scopeState=journalScopeState(scope,sync.personUid);scopeState.last_scan_at=new Date().toISOString();if(scope==='global')scopeState.access_ai_id=aiId;else scopeState.ai_id=aiId;
  const locals=(scope==='global'?wikiEntries():journalEntries().filter(x=>x.directory_uid===person?.directory_uid)).filter(x=>!x.archived||x.deleted_at||scope==='global');sync.plan=window.LifelineJournalSync.buildSynchronizationPlan({locals,remotes:result.entries,scope,initialized:scopeState.initialized});sync.plan.entriesScanned=result.entries.length;sync.busy=false;sync.progress='Comparison ready. Review every action before applying.';render();
}
...
async function applyJournalSynchronization(){const sync=state.journalSync,plan=sync.plan;if(!plan||sync.busy)return;document.querySelectorAll('[data-journal-resolution]').forEach(select=>{if(select.value){const i=Number(select.dataset.journalResolution);plan.operations[i]=window.LifelineJournalSync.resolveConflict(plan.operations[i],select.value);}});if(plan.operations.some(x=>['conflict','uncertain_match'].includes(x.action))){sync.error='Resolve or exclude every conflict before applying.';render();return;}sync.busy=true;sync.error='';const cfg=ensureJournalSyncConfig();const scopeState=journalScopeState(sync.scope,sync.personUid);await saveBridge('Checkpoint journal synchronization plan',true);
  try{for(let index=0;index<plan.operations.length;index++){const op=plan.operations[index],now=new Date().toISOString();sync.progress=`${index+1}/${plan.operations.length}: ${op.action.replaceAll('_',' ')}`;render();cfg.pending_operation={scope:sync.scope,index,action:op.action,started_at:now};await saveBridge('Checkpoint pending journal operation',true);
    ...
    scopeState.initialized=true;scopeState.last_sync_at=new Date().toISOString();cfg.pending_operation=null;sync.progress='Synchronization completed and verified.';await saveBridge('Complete journal synchronization',true);sync.open=false;
