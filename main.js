...
function renderWikiView() {
  ...
  const keywords = (Array.isArray(current?.keywords) ? current.keywords : []).map((keyword) => `<span>${escapeHtml(keyword)}</span>`).join('');
