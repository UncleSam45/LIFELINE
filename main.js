...
async function writeBridgeConfig(reason, retryOnShaMismatch = true) {
  const expectedDirectoryUids = entries().map((entry) => String(entry.directory_uid || '').trim()).filter(Boolean);
  ...
  try {
    ...
    await verifyRootBridgeConfig(expectedDirectoryUids);
...
async function verifyRootBridgeConfig(expectedDirectoryUids) {
  const saved = await readGithubContentFile(BRIDGE_PATH);
  const savedUids = new Set((saved.config.directory_entries || []).map((entry) => String(entry?.directory_uid || '').trim()).filter(Boolean));
  const missing = expectedDirectoryUids.filter((uid) => !savedUids.has(uid));
